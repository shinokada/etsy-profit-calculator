<script lang="ts">
	...
	let dicountPlaceholder: string = $derived(
		dicountDollar ? 'Enter discount amount' : 'Enter discount percentage'
	);
	...
	let discountPrice: number = $derived(
		dicountDollar ? discountValue : (salesPrice + shippingPrice) * (discountValue / 100)
	);

	let adPrice: number = $derived(
		adDollar ? adValue : (salesPrice + shippingPrice) * (adValue / 100)
	);
	...
	let paymentProcessingFee: number = $derived(
		totalRevenue * PAYMENT_PROCESSING_PERCENTAGE + PAYMENT_PROCESSING_FIXED_FEE
	);

	let totalFees: number = $derived(
		calculateTotalFees(salesPrice, shippingPrice, discountValue / 100, vatEtsyPercentage / 100) +
			adPrice +
			offsiteAd
	);
	...
	let totalCoGSAndShipping: number = $derived(
		calculateTotalCoGSAndShipping(costOfItem, costOfShipping) + vatCosts
	);

	let totalCosts: number = $derived(
		calculateTotalCost(costOfItem, costOfShipping, vatCostPercentage / 100) + totalFees + vatEtsy
	);

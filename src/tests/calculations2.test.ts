...
describe('2: calculateVatOnCosts', () => {
	it('2: calculates vat on costs', () => {
		expect(Math.round(calculateVatOnCosts(costOfItem, shippingCostForOrder, vatPercentageOnCostAndShipping) * 100) / 100).toBeCloseTo(2.77, 2);
	});
});
...
describe('2: calculateTotalFees', () => {
  it('2: calculates total fees', () => {
    expect(calculateTotalFees(salesPrice, shippingPrice, discountPrice, etsyAds, offsiteAds)).toBeCloseTo(3.253, 3);
  });
});
...
describe('2: calculateEtsyTransactionFee', () => {
  it('2: calculates etsy transaction fee', () => {
    expect(calculateEtsyTransactionFee(salesPrice, shippingPrice, discountPrice)).toBeCloseTo(1.918, 2);
  });
});
...
describe('2: calculatePaymentProcessingFee', () => {
  it('2: calculates payment processing fee', () => {
    expect(calculatePaymentProcessingFee(salesPrice, shippingPrice, discountPrice)).toBeCloseTo(1.135, 3);
  });
});
